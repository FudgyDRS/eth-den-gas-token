...
contract Derivadata is ERC721A, Ownable, ERC721ABurnable, ERC721AQueryable {
    ...
    event changedMetadata(uint256 tokenId, string oldData, string newData);
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721A, IERC721A) returns (string memory) {
        if (!_exists(tokenId)) revert URIQueryForNonexistentToken();
        if (keccak256(abi.encodePacked(_tokenURI[tokenId])) == keccak256(abi.encodePacked(""))) return string(abi.encodePacked(_baseURI(), uint2str(tokenId)));
     ...
     * @dev Returns the IPFS URI of a specific NFT
     */
    function setTokenURI(uint256 tokenId, string memory newData) public {
        if (!_exists(tokenId)) revert URIQueryForNonexistentToken();
        require(isApprovedForAll(ownerOf(tokenId), msg.sender) || ownerOf(tokenId) == msg.sender);

        _tokenURI[tokenId] = newData;
    }
